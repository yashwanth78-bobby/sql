...
select department,count(worker_id) as cnt from worker group by department having department='HR';
...
select * from worker as w1 where 1=(select count(distinct(salary))  from worker as w2 where w2.salary>w1.salary);
